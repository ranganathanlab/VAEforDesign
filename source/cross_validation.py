...
class VAE(nn.Module):
    ...
    def reparam(self, mu, logvar): 
        ...
        if self.training:
            ...
            eps = Variable(std.data.new(std.size()).normal_()) # torch variable
...
def plot_training(loss_train, loss_val, figname):
...
path='Inputs/'
q_n=np.load(path + 'q_n.npy') # number of possibilities on each position
v_traj_onehot = np.loadtxt(path + 's1a_onehot.txt')
print(v_traj_onehot.shape)
print('number of possible amino acids in each position q_n = \n',q_n)
print('length(q_n) = ',len(q_n))

for i in range(q_n.shape[0]):
    position=np.sum(q_n[:i])
    a=v_traj_onehot[0][position:position+q_n[i]].astype(int)
...
# training VAEs with d=1...dMax to determine appropriate d
...
for partition in range(nPartition): # k-fold cross validation
    ...
    for d in range(1,dMax+1): # iterating over different dimensionalities of latent space
        ...
        plot_training(loss_train,loss_val,'d_crossValidation_k='+str(partition+1)+'_d='+str(d)+'_loss.png')
        plot_training(loss_train_MSE,loss_val_MSE,'d_crossValidation_k='+str(partition+1)+'_d='+str(d)+'_lossMSE.png')
...
if showPlots:
    plt.draw()
    plt.show()
fig.savefig('d_crossValidation.png', dpi=300)
plt.close()
...
if showPlots:
    plt.draw()
    plt.show()
fig.savefig('d_crossValidation_MSE.png', dpi=300)
plt.close()